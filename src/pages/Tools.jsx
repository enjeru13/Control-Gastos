...
import { ArrowLeftRight, ChevronLeft, ChevronRight, TrendingUp, ArrowDown, ArrowUp } from 'lucide-react'
...
function useCurrencyRates(base) {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.frankfurter.app/latest?from=${base}&to=USD,COP,EUR,VES`)
      ...
      .then((data) => {
        const r = { ...data.rates }
        if (!r.VES) r.VES = base === 'USD' ? 36.5 : (data.rates.USD ?? 1) * 36.5
        setRates(r)
        setLastUpdated(new Date())
      })
      .catch(() => {
        setRates({ USD: 1, COP: 3850, EUR: 0.93, VES: 36.5 })
      })
...
function CurrencyCalculator() {
  ...
  return (
    <section className="flex flex-col gap-3">
      <h2 className="text-base font-bold text-on-surface">Calculadora de Divisas</h2>
      ...
      <div className="bg-surface rounded-2xl shadow-card p-5 flex flex-col gap-4 relative">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex justify-between text-xs text-outline mt-0.5 px-1">
            ...
            <button onClick={swap} className="text-primary font-semibold hover:underline">
              Invertir
            </button>
          </div>
        </div>
        ...
        {lastUpdated && (
          <div className="flex items-center gap-1.5 text-[10px] text-outline">
            <TrendingUp size={11} />
            Tasa actualizada: {lastUpdated.toLocaleTimeString('es', { hour: '2-digit', minute: '2-digit' })}
          </div>
        )}
      </div>
    </section>
...
const DAY_TXS = {
  '2026-05-20': [
    { id: 1, description: 'Transferencia Zelle', amount: 150, currency: 'USD', type: 'income', time: '10:30 AM' },
    { id: 2, description: 'Pago Servicios', amount: 450, currency: 'VES', type: 'expense', time: '02:15 PM' },
  ],
}
...
function FinancialCalendar() {
  ...
  const monthLabel = new Intl.DateTimeFormat('es', { month: 'long', year: 'numeric' }).format(
  ...
  return (
    <section className="flex flex-col gap-3">
      <h2 className="text-base font-bold text-on-surface">Calendario Financiero</h2>
      ...
      <div className="bg-surface rounded-2xl shadow-card p-5 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-7 text-center">
          {DAYS.map((d) => (
            <div key={d} className="text-[10px] font-bold text-outline pb-1">{d}</div>
          ))}
      ...
      {selected && (
        <div className="flex flex-col gap-2">
          <h4 className="text-xs font-semibold text-on-surface-variant px-1 capitalize">{selectedLabel}</h4>
                ...
                <div key={tx.id}>
                  ...
                  <div className="flex items-center justify-between p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-sm font-semibold text-on-surface">{tx.description}</div>
