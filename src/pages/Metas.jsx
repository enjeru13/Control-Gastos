...
import {
  Plus, MoreVertical,
  Smartphone, Plane, Shield,
  BookOpen, Star, TrendingUp, PiggyBank,
  CreditCard, User, Users, Home, Car, Sofa, Receipt, AlertCircle,
  Loader2, Trash2,
} from 'lucide-react'
...
function Field({ label, error, children }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-xs font-semibold text-on-surface-variant tracking-wide">{label}</label>
      {children}
...
function AportarSheet({ open, goal, onClose, onSave }) {
  ...
  return (
    <BottomSheet open={open} title={goal ? `Aportar a "${goal.name}"` : 'Aportar'} onClose={onClose}>
      <form onSubmit={handle} className="flex flex-col gap-5">
        {goal && (
          <div className="bg-primary-container/30 rounded-2xl p-4 flex justify-between items-center">
            <div>
              ...
              <p className="text-base font-bold text-on-surface">{fmtAmt(goal.current_amount, goal.currency)}</p>
            </div>
            <div className="text-right">
              <p className="text-xs text-on-surface-variant mb-0.5">Objetivo</p>
              <p className="text-base font-bold text-primary">{fmtAmt(goal.target_amount, goal.currency)}</p>
            </div>
          </div>
        )}
        <Field label="Monto a aportar" error={error}>
          <input
            ...
            placeholder="0.00" className={InputCls(!!error)} autoFocus
          />
        </Field>
        <button type="submit" disabled={saving}
          className="w-full py-4 rounded-xl bg-gradient-to-b from-primary to-surface-tint text-on-primary text-sm font-bold shadow-card active:scale-[0.98] transition-all disabled:opacity-60 flex items-center justify-center gap-2">
          {saving ? <><Loader2 size={16} className="animate-spin" />Guardando...</> : 'Registrar Aporte'}
        </button>
      </form>
    </BottomSheet>
  )
...
function AbonarSheet({ open, debt, onClose, onSave }) {
  ...
  return (
    <BottomSheet open={open} title={debt ? `Abonar a "${debt.name}"` : 'Abonar'} onClose={onClose}>
      <form onSubmit={handle} className="flex flex-col gap-5">
        {debt && (
          <div className="bg-error-container/30 rounded-2xl p-4 flex justify-between items-center">
            ...
            <div className="text-right">
              ...
              <p className="text-base font-bold text-success">{fmtAmt(debt.paid_amount, debt.currency)}</p>
            </div>
          </div>
        )}
...
const BLANK_GOAL = { name: '', target_amount: '', currency: 'USD', target_date: '', icon: 'PiggyBank' }
...
function AddGoalSheet({ open, onClose, onSave, defaultValues = null }) {
  ...
  // When sheet opens, populate from defaultValues (wishlist convert) or blank
  useEffect(() => {
    if (open) {
  ...
  async function handle(e) {
    ...
    if (!form.target_amount || +form.target_amount <= 0) errs.target_amount = 'Monto inválido'
    if (Object.keys(errs).length) { setErrors(errs); return }
    setSaving(true)
    try {
      await onSave({
        name: form.name.trim(), target_amount: +form.target_amount, current_amount: 0,
        currency: form.currency, target_date: form.target_date || null,
        icon: form.icon, color: '#1b667c',
      })
      setForm(BLANK_GOAL)
      onClose()
      ...
      <form onSubmit={handle} className="flex flex-col gap-5">
        ...
        <Field label="Nombre de la meta" error={errors.name}>
          <input value={form.name} onChange={(e) => set('name', e.target.value)}
            placeholder="Ej. Viaje a Europa" className={InputCls(errors.name)} autoFocus />
        </Field>
        <Field label="Monto objetivo" error={errors.target_amount}>
          <input type="number" step="0.01" min="0" value={form.target_amount}
            onChange={(e) => set('target_amount', e.target.value)} placeholder="0.00"
          ...
          <div className="flex gap-2">
            {CURRENCIES.map((c) => (
              <button key={c} type="button" onClick={() => set('currency', c)}
                className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all active:scale-95 ${form.currency === c ? 'bg-primary-container text-on-primary-container shadow-sm' : 'bg-surface-container-low border border-outline-variant text-on-surface-variant'}`}>{c}</button>
            ))}
          </div>
        </Field>
        ...
        {errors.general && <p className="text-xs text-error bg-error-container/30 rounded-xl px-4 py-3 text-center">{errors.general}</p>}
        <button type="submit" disabled={saving}
          className="w-full py-4 rounded-xl bg-gradient-to-b from-primary to-surface-tint text-on-primary text-sm font-bold shadow-card active:scale-[0.98] transition-all disabled:opacity-60 flex items-center justify-center gap-2">
          {saving ? <><Loader2 size={16} className="animate-spin" />Guardando...</> : 'Crear Meta'}
        </button>
      </form>
    </BottomSheet>
  )
...
function AddDebtSheet({ open, onClose, onSave }) {
  const [form,   setForm]   = useState({ name: '', type: 'credit_card', total_amount: '', paid_amount: '0', currency: 'USD', interest_rate: '', due_date: '' })
  const [saving, setSaving] = useState(false)
  const [errors, setErrors] = useState({})
  ...
  async function handle(e) {
    ...
    const errs = {}
    if (!form.name.trim()) errs.name = 'Requerido'
    if (!form.total_amount || +form.total_amount <= 0) errs.total_amount = 'Monto inválido'
    if (+form.paid_amount < 0) errs.paid_amount = 'No puede ser negativo'
    if (Object.keys(errs).length) { setErrors(errs); return }
    setSaving(true)
    try {
      await onSave({
        name: form.name.trim(), type: form.type, total_amount: +form.total_amount,
        paid_amount: +form.paid_amount || 0, currency: form.currency,
        interest_rate: form.interest_rate ? +form.interest_rate : null,
        due_date: form.due_date || null,
        color: DEBT_TYPE_META[form.type]?.color ?? '#ba1a1a',
      })
      setForm({ name: '', type: 'credit_card', total_amount: '', paid_amount: '0', currency: 'USD', interest_rate: '', due_date: '' })
      onClose()
    } catch (err) { setErrors({ general: err.message }) }
    finally { setSaving(false) }
  }
    ...
    <BottomSheet open={open} title="Registrar Deuda" onClose={onClose}>
      <form onSubmit={handle} className="flex flex-col gap-5">
        <Field label="Nombre" error={errors.name}>
          <input value={form.name} onChange={(e) => set('name', e.target.value)}
            placeholder="Ej. Tarjeta Visa" className={InputCls(errors.name)} autoFocus />
        </Field>
        <Field label="Tipo de deuda">
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(DEBT_TYPE_META).map(([key, { label, Icon, color }]) => (
              <button key={key} type="button" onClick={() => set('type', key)}
                ...
                <Icon size={15} style={{ color: form.type === key ? color : undefined }} />
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Interés anual % (opcional)">
            <input type="number" step="0.01" min="0" value={form.interest_rate}
              onChange={(e) => set('interest_rate', e.target.value)} placeholder="Ej. 24.5"
              className={InputCls(false)} />
          </Field>
        ...
        <button type="submit" disabled={saving}
          className="w-full py-4 rounded-xl text-on-primary text-sm font-bold shadow-card active:scale-[0.98] transition-all disabled:opacity-60 flex items-center justify-center gap-2"
          style={{ background: 'linear-gradient(180deg, #ba1a1a 0%, #93000a 100%)' }}>
          {saving ? <><Loader2 size={16} className="animate-spin" />Guardando...</> : 'Registrar Deuda'}
        </button>
      </form>
    </BottomSheet>
  )
...
function AddWishlistSheet({ open, onClose, onSave }) {
  const [form,   setForm]   = useState({ name: '', price: '', currency: 'USD', priority: 'medium', description: '' })
  const [saving, setSaving] = useState(false)
  const [errors, setErrors] = useState({})
  ...
  async function handle(e) {
    ...
    try {
      await onSave({
        name: form.name.trim(), price: form.price ? +form.price : null,
        currency: form.currency, priority: form.priority,
        description: form.description || null,
      })
      setForm({ name: '', price: '', currency: 'USD', priority: 'medium', description: '' })
      onClose()
    } catch (err) { setErrors({ general: err.message }) }
    finally { setSaving(false) }
  }
  ...
  return (
    <BottomSheet open={open} title="Agregar a Wishlist" onClose={onClose}>
      <form onSubmit={handle} className="flex flex-col gap-5">
        <Field label="Nombre" error={errors.name}>
          <input value={form.name} onChange={(e) => set('name', e.target.value)}
            placeholder="Ej. Silla Ergonómica" className={InputCls(errors.name)} autoFocus />
        </Field>
        <Field label="Prioridad">
          <div className="flex gap-2">
            {[['high', 'Alta'], ['medium', 'Media'], ['low', 'Baja']].map(([key, label]) => (
              <button key={key} type="button" onClick={() => set('priority', key)}
                className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all active:scale-95 ${
                  form.priority === key
                    ? key === 'high' ? 'bg-error-container text-on-error-container shadow-sm'
                      : key === 'medium' ? 'bg-primary-container text-on-primary-container shadow-sm'
        ...
        <Field label="Descripción (opcional)">
          <input value={form.description} onChange={(e) => set('description', e.target.value)}
            placeholder="Ej. Para la oficina en casa" className={InputCls(false)} />
        </Field>
        {errors.general && <p className="text-xs text-error bg-error-container/30 rounded-xl px-4 py-3 text-center">{errors.general}</p>}
        <button type="submit" disabled={saving}
          className="w-full py-4 rounded-xl bg-gradient-to-b from-primary to-surface-tint text-on-primary text-sm font-bold shadow-card active:scale-[0.98] transition-all disabled:opacity-60 flex items-center justify-center gap-2">
          {saving ? <><Loader2 size={16} className="animate-spin" /> Guardando...</> : 'Agregar a Wishlist'}
        </button>
      </form>
    </BottomSheet>
  )
    ...
    <div className="bg-surface rounded-2xl p-5 shadow-card border border-surface-container hover:shadow-overlay transition-shadow duration-300 flex flex-col gap-4 relative">
      <div className="flex justify-between items-start">
        ...
        <div className="relative">
          ...
          {menu && (
            <div className="absolute right-0 top-8 bg-surface border border-surface-container rounded-xl shadow-overlay z-10 overflow-hidden min-w-[150px]">
              <button onClick={() => { onAportar(goal); setMenu(false) }}
              ...
              <button onClick={() => { onDelete(goal.id); setMenu(false) }}
      ...
      <div>
        <div className="flex justify-between items-end mb-2">
          <span className="text-xl font-bold font-currency text-on-surface">{fmtAmt(goal.current_amount, goal.currency)}</span>
          <span className="text-xs text-on-surface-variant">de {fmtAmt(goal.target_amount, goal.currency)}</span>
        </div>
        <div className="w-full h-3 bg-primary/10 rounded-full overflow-hidden">
          <div className="h-full rounded-full bg-gradient-to-r from-tertiary-fixed-dim to-primary transition-all duration-700"
            style={{ width: `${pct}%` }} />
        </div>
...
function DebtCard({ debt, onDelete, onAbonar }) {
  ...
  const pct  = debt.total_amount > 0 ? Math.min(Math.round((debt.paid_amount / debt.total_amount) * 100), 100) : 0
        ...
        <div className="flex-1 p-5 flex flex-col gap-4">
          <div className="flex justify-between items-start">
            ...
            <div className="relative">
              ...
              {menu && (
                <div className="absolute right-0 top-8 bg-surface border border-surface-container rounded-xl shadow-overlay z-10 overflow-hidden min-w-[165px]">
                  <button onClick={() => { onAbonar(debt); setMenu(false) }}
                    className="flex items-center gap-2 w-full px-4 py-3 text-sm text-primary hover:bg-primary-container/20 transition-colors">
                    <Receipt size={14} /> Registrar abono
                  </button>
                  <button onClick={() => { onDelete(debt.id); setMenu(false) }}
                    className="flex items-center gap-2 w-full px-4 py-3 text-sm text-error hover:bg-error-container/20 transition-colors">
                    <Trash2 size={14} /> Eliminar
                  </button>
                </div>
          ...
          <div className="flex justify-between items-end">
            <div>
              ...
              <p className="text-xl font-bold font-currency" style={{ color }}>{fmtAmt(remaining, debt.currency)}</p>
            </div>
            <div className="text-right">
              <p className="text-xs text-on-surface-variant mb-0.5">Pagado</p>
              <p className="text-sm font-bold text-success font-currency">{fmtAmt(debt.paid_amount, debt.currency)}</p>
            </div>
          </div>

          <div>
            <div className="w-full h-3 bg-error/10 rounded-full overflow-hidden">
              <div className="h-full rounded-full bg-gradient-to-r from-error to-success transition-all duration-700"
                style={{ width: `${pct}%` }} />
            </div>
            <div className="flex justify-between mt-1.5">
              <span className="text-[10px] text-on-surface-variant">Total {fmtAmt(debt.total_amount, debt.currency)}</span>
    ...
    <div className="flex items-center justify-between p-4 rounded-xl hover:bg-surface-container-low transition-colors group border-b border-surface-container-high/50 last:border-0">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="min-w-0">
          <h4 className="text-sm font-bold text-on-surface group-hover:text-primary transition-colors truncate">{item.name}</h4>
          <div className="flex items-center gap-2 mt-1">
            <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${badge.className}`}>{badge.label}</span>
            {item.description && <span className="text-xs text-on-surface-variant truncate">{item.description}</span>}
          </div>
        </div>
      </div>
      <div className="flex items-center gap-2 shrink-0 ml-2">
        {item.price && (
          <span className="text-base font-bold text-on-surface">{fmtAmt(item.price, item.currency)}</span>
        )}
        <div className="relative">
          <button onClick={() => setMenu((m) => !m)}
            className="text-outline hover:text-on-surface transition-colors p-1 rounded-lg hover:bg-surface-container">
          ...
          {menu && (
            <div className="absolute right-0 top-8 bg-surface border border-surface-container rounded-xl shadow-overlay z-10 overflow-hidden min-w-[170px]">
              <button onClick={() => { onConvert(item); setMenu(false) }}
                className="flex items-center gap-2 w-full px-4 py-3 text-sm text-primary hover:bg-primary-container/20 transition-colors">
...
export default function Metas() {
  ...
  const { goals, loading: loadingGoals, addGoal,  updateGoal, deleteGoal } = useSavingsGoals()
  ...
  const nearestGoal = goals.length > 0
    ? goals.reduce((best, g) =>
        (g.current_amount / g.target_amount) > (best.current_amount / best.target_amount) ? g : best)
    : null
  const nearestPct      = nearestGoal ? Math.round((nearestGoal.current_amount / nearestGoal.target_amount) * 100) : 0
    ...
    <div className="flex flex-col gap-6">
      ...
      {(goals.length > 0 || debts.length > 0) && (
        <section className="flex flex-col gap-3">
          ...
          <div className="grid gap-3" style={{ gridTemplateColumns: debts.length > 0 ? '1fr 1fr' : '1fr' }}>
            ...
            <div className="bg-surface rounded-2xl p-4 shadow-card flex items-center gap-3 border border-surface-container-high/40">
              <div className="w-11 h-11 rounded-full flex items-center justify-center shrink-0"
                ...
                <nearestIconMeta.Icon size={20} style={{ color: nearestIconMeta.color }} />
              </div>
              <div className="min-w-0">
                ...
                {loadingGoals ? (
                  ...
                  <>
                    <div className="text-sm font-bold text-on-surface truncate">{nearestGoal.name}</div>
                    <div className="text-xs font-semibold text-primary mt-0.5">{nearestPct}% completado</div>
                  </>
                ) : (
                  <div className="text-xs text-on-surface-variant">Sin metas aún</div>
                )}
              </div>
            </div>
              ...
              <div className="bg-error-container/40 rounded-2xl p-4 shadow-card flex items-center gap-3 border border-error-container">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-xs font-semibold text-on-surface-variant mt-0.5">
                    {loadingDebts ? '' : `${debts.length} deuda${debts.length !== 1 ? 's' : ''} activa${debts.length !== 1 ? 's' : ''}`}
                  </div>
                </div>
              </div>
            )}
        ...
        {TABS.map(({ key, label }) => (
          <button key={key} onClick={() => setActiveTab(key)}
            className={['flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all',
              activeTab === key ? 'bg-surface shadow-sm text-primary' : 'text-on-surface-variant hover:text-on-surface',
            ].join(' ')}>
            {label}
          </button>
        ))}
        ...
        <section className="flex flex-col gap-4">
          ...
          {loadingGoals ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {goals.map((g) => (
                <GoalCard key={g.id} goal={g} onDelete={deleteGoal} onAportar={setAportarGoal} />
              ))}
              <button
                onClick={() => setSheet('goal')}
              ...
              >
                <Plus size={18} />
                <span className="text-sm font-semibold">Nueva meta de ahorro</span>
              </button>
            </div>
          )}
        </section>
        ...
        <section className="flex flex-col gap-4">
            ...
            <div className="bg-error-container/30 border border-error-container rounded-2xl p-4 flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-sm font-bold text-success font-currency">
                  {fmtAmt(debts.reduce((s, d) => s + Number(d.paid_amount), 0), 'USD')}
                </p>
              </div>
            </div>
          )}
          ...
          {loadingDebts ? (
            ...
            <div className="flex flex-col gap-3">
              {debts.map((d) => (
                <DebtCard key={d.id} debt={d} onDelete={deleteDebt} onAbonar={setAbonarDebt} />
              ))}
              <button
                onClick={() => setSheet('debt')}
              ...
              >
                <Plus size={16} />
                <span className="text-sm font-semibold">Registrar nueva deuda</span>
              </button>
            </div>
          )}
        </section>
        ...
        <section className="flex flex-col gap-4">
          ...
          {loadingWish ? (
            ...
            <div className="bg-surface rounded-3xl shadow-card border border-surface-container overflow-hidden">
              {items.length === 0 && (
                <p className="text-sm text-on-surface-variant text-center py-8">Sin items aún</p>
              )}
              {items.map((item) => (
                <WishlistItem key={item.id} item={item} onDelete={deleteItem} onConvert={handleConvert} />
              ))}
              <div className="p-3">
                <button
                  onClick={() => setSheet('wishlist')}
                ...
                >
                  <Plus size={16} />
                  <span className="text-sm font-semibold">Agregar a wishlist</span>
                </button>
              </div>
            </div>
          )}
      ...
      <AddGoalSheet
        ...
        onClose={() => { setSheet(null); setConvertItem(null) }}
        onSave={convertItem ? handleGoalSaveAndConvert : addGoal}
      />
      <AddDebtSheet     open={sheet === 'debt'}     onClose={() => setSheet(null)} onSave={addDebt} />
      <AddWishlistSheet open={sheet === 'wishlist'} onClose={() => setSheet(null)} onSave={addItem} />
